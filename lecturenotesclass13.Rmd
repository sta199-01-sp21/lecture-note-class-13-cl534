...
Compared to intervals, point estimates are less likely to capture true value (trade off between accuracy and precision.)
...
- The term **bootstrapping** comes from the phrase "pulling oneself up 
  ...
  (use when you have limited amt of data)
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-cl534/manhattan.csv")
```
...
**Interpretation**: We are 95% confident that the true mean price (true parameter) is contained between  $2299 and $2977.
...
**Interpretation**: We are 95% confident that the true median price (true parameter) is contained between  $2162 and $2875.
...
- Population (observations in your analysis): Mice not on treatment
...
**Interpretation**: We are 95% confident that the true proportion of mice who died while not on the treatment is between 69% and 79.3%.
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(mice_lived, level = .90)
ci_mice_lived
```
...
We are 90% confident that the true proportion of mice that lived while on treatment is between 55.2% and 65.1%. 
